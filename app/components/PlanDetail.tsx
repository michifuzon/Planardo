...
import {
  ...
  addPlanComment, cancelPlan, createPlanInvite, deleteChecklistItem, deleteExpense, deletePlanItem, deletePlanMessage, deletePoll,
...
export default function PlanDetail({id,onBack,onDeleted,onOpenProfile,isAdmin,friends=[],groups=[]}:{id:string;onBack:()=>void;onDeleted?:()=>void;onOpenProfile?:(id:string)=>void;isAdmin?:boolean;friends?:Profile[];groups?:Group[]}) {
  ...
  async function send(){if(!composer.trim())return;setChatError("");try{await sendPlanMessage(id,composer.trim());setComposer("");refresh()}catch(error){setChatError(error instanceof Error?error.message:"No se pudo enviar el mensaje.")}}
  async function delMessage(messageId:string){setChatError("");try{await deletePlanMessage(messageId);refresh()}catch(error){setChatError(error instanceof Error?error.message:"No se pudo borrar el mensaje.")}}
  ...
  return <section className="plan-detail">
    ...
    <AnimatePresence>{editOpen&&editForm&&<motion.div className="modal-backdrop" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={e=>e.target===e.currentTarget&&setEditOpen(false)}><motion.div className="modal edge" initial={{opacity:0,y:30,scale:.97}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:30,scale:.97}} transition={{type:"spring",damping:26,stiffness:300}}>
      ...
      <div className="form">
        ...
        <label className="field"><span><MapPin size={17}/> Link de ubicación</span><input type="text" inputMode="url" placeholder="https://maps.google.com/…" value={editForm.location_url} onChange={e=>setEditForm({...editForm,location_url:e.target.value})}/></label>
